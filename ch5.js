...
function dominantDirection(text) {
  ...
  let dominantScriptCount = scripts.reduce((acc, {count}, ind, arr) => {
    return acc > count ? acc : count;
  }, scripts[0].count);

  let dominantScript = scripts.find(s => s.count === dominantScriptCount);

  return SCRIPTS.find(s => s.name === dominantScript.name).direction;
}
